...
# Clean any duplicate setup lines from earlier setup attempts.
while 'import gzip\nimport gzip\n' in s:
    s=s.replace('import gzip\nimport gzip\n','import gzip\n',1)
while 'from zoneinfo import ZoneInfo\nfrom zoneinfo import ZoneInfo\n' in s:
    s=s.replace('from zoneinfo import ZoneInfo\nfrom zoneinfo import ZoneInfo\n','from zoneinfo import ZoneInfo\n',1)
dup='''STUDY_START = dt.date(2026, 8, 31)
...
STUDY_DIR = Path("study/market_snapshots")
'''
s=s.replace(dup,'''STUDY_START = dt.date(2026, 8, 31)
STUDY_END = dt.date(2026, 9, 14)
STUDY_DIR = Path("study/market_snapshots")
''')
...
if 'STUDY_START = dt.date(2026, 8, 31)' not in s:
...
if 'def write_study_snapshot(valid, surfaced):' not in s:
    ...
    payload = {"capturedAt":now_utc.isoformat(),"melbourneDate":mel.date().isoformat(),"studyStart":STUDY_START.isoformat(),"studyEnd":STUDY_END.isoformat(),"companyCount":len(stocks),"selectedCount":len(selected),"stocks":stocks}
...
call='    write_study_snapshot(valid, surfaced)\n'
if '\n'+call not in s:
    if needle not in s: raise SystemExit('surfaced anchor missing')
    s=s.replace(needle,needle+call,1)

p.write_text(s,encoding='utf-8')
print('market-wide study capture verified and patched')
