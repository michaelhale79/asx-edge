...
if old not in s: raise SystemExit('HOLD reason point not found')
s=s.replace(old,new,1)
...
# Best Opportunity: use cleaned disclosure evidence and reward/penalise the separately capped AI signal.
old='ev=Number(x.evidenceScore??50),er=Number(x.evidenceRisk??50),md=Number(x.managementDeliveryScore??50),stretch='
new='ev=Number(x.qualityEvidenceScore??x.evidenceScore??50),er=Number(x.qualityEvidenceRisk??x.evidenceRisk??50),md=Number(x.managementCredibilityScore??x.managementDeliveryScore??50),ai=Number(x.aiEventAdjustment||0),stretch='
if old not in s: raise SystemExit('best opportunity variables not found')
s=s.replace(old,new,1)
old='+shortAdj*.25-stretch};'
new='+shortAdj*.25+ai*1.6-stretch};'
if old not in s: raise SystemExit('best opportunity formula point not found')
s=s.replace(old,new,1)
...
if old not in s:
    # Older/current version builds global trends inline; insert beside a reliable evidence assignment instead.
    anchor='$("#detailEvidence").innerHTML='
    pos=s.find(anchor)
    if pos<0: raise SystemExit('detail render anchor not found')
    s=s[:pos]+'$("#detailAIEvents").innerHTML=aiEventHtml(s);'+s[pos:]
else:
    s=s.replace(old,old+'$("#detailAIEvents").innerHTML=aiEventHtml(s);',1)
